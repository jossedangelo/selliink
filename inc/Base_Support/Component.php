...
class Component implements Component_Interface, Templating_Component_Interface
{
	...
	public function action_essential_theme_support()
	{
		...
		if (function_exists('buddypress')) {
			if (class_exists('Redux') && !empty($socialv_options['socialv_default_avatar']['url'])) {
			...
			if (class_exists('Redux') && !empty($socialv_options['socialv_default_cover_image']['url'])) {
	...
	public function filter_body_classes_add_hfeed(array $classes): array
	{
		...
		$is_header = $this->is_socialv_header();
		if (function_exists('get_field') && class_exists('Redux')) {
			$is_default_header = $this->is_socialv_default_header();
		}
		$display_sidebar = (class_exists('Redux') && $socialv_options['header_display_side_area'] == 'yes') ? true : false;
		if ($display_sidebar == true && $is_default_header == false) {
			$classes[] = 'sidebar-active';
		}
		if ($is_header) {
			if (!$is_default_header) {
				$classes[] = 'verticle-header';
			}
	...
	public function is_socialv_breadcrumb()
    {
        ...
        $breadcrumb_page_option = !empty($page_id) ? get_post_meta($page_id, 'display_banner', true) : 'default';
        ...
        } elseif ($socialv_options['display_banner'] == "no") { 
            $is_bredcrumb = false;
        }
	...
	public function is_socialv_default_header()
    {
