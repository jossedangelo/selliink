...
class Merlin
{
	...
	function __construct($config = array(), $strings = array())
	{
		...
		// Get TGMPA.
		if (class_exists('Socialv_TGM_Plugin_Activation')) {
			$this->tgmpa = isset($GLOBALS['tgmpa']) ? $GLOBALS['tgmpa'] : Socialv_TGM_Plugin_Activation::get_instance();
		}
		...
		add_action('init', array($this, 'socialv_download_file'));
		add_action('socialv_stop_redirect_action', array($this, 'socialv_stop_redirect_action'));
	}
		...
		<body class="merlin__body merlin__body--<?php echo esc_attr($current_step); ?>">
			...
			<div class="wizard-progress current-step-<?php echo esc_attr($this->step); ?>">
				<div class="step <?php echo esc_attr($class_welcome); ?>">
					<?php echo esc_html__("Welcome", "socialv"); ?>
					<div class="node">
	...
	protected function welcome()
	{
		...
		$php_configs = array(
			...
			'memory_limit' => array('require' => '512M', 'current' => ini_get('memory_limit')),
	...
	?>
		...
		<div class="merlin__content--transition">
			...
			<div class="content-right">
				<div class="content-right-inner">
					...
					<table class="table">
						<tr class="heading">
							<th><?php esc_html_e('PHP Configuration', 'socialv') ?></th>
							<th><?php esc_html_e('Recommended (Min)', 'socialv') ?></th>
							<th><?php esc_html_e('Existing', 'socialv') ?></th>
						</tr>
						...
						?>
							<tr class="">
								...
								<?php
								if (rtrim($php_config['require'], 'M') > rtrim($php_config['current'], 'M')) {
									$requirements_met = false;
								} ?>
								<td><code style="<?php rtrim($php_config['require'], 'M') <= rtrim($php_config['current'], 'M') ? '' : esc_attr_e('color:red') ?>"><?php esc_html_e($php_config['current']) ?></code></td>
							</tr>
						<?php
						}
						?>
					...
					?>
						<div class="error-message mt-3">
							<?php
							$link_url = 'https://jetpack.com/blog/wordpress-php-ini/';
							$error_message = __('Dear User, please check the provided link for modifying your server configurations. Alternatively, reach out to your service provider for assistance.', 'socialv');
							$error_message_with_link = str_replace('link', '<a href="' . esc_url($link_url) . '" target="_blank">link</a>', $error_message);
		...
		<footer class="merlin__content__footer">
			...
			<?php } else { ?>
				<b class="text-danger">
					<?php esc_html_e('Please match your server configuration with the theme`s required configuration.', 'socialv'); ?>
				</b>
			<?php	} ?>
		...
		<div class="merlin__content--transition">
			...
			<div class="content-right">
				<div class="content-right-inner">
					...
					<p>
						<strong><?php esc_html_e("About child theme", "socialv"); ?></strong><br>
						<?php
						esc_html_e("A child theme allows you to change small aspects of your site's appearance yet still preserve your theme's look and functionality. To understand how child themes work it is first important to understand the relationship between parent and child themes.", "socialv");
						?>
		...
		<div class="merlin__content--transition">
			...
			<div class="content-right">
				...
				<div class="content-right-inner">
					...
					<?php if ($count) :
						$required = $iqonic = $woocommerce = $learnPress = $gamipress = $pmp = $bbpress = $other = '';  ?>
								...
								if ($plugin['type'] == 'required') {
									$required .= '<li data-slug="' . esc_attr($slug) . '">
										...
										<label for="default_plugins_' . esc_attr($slug) . '">
											...
											<span class="badge">
										<span class="hint--top" aria-label="' . esc_html__('Required', 'merlin-wp') . '">
											' . esc_html__('req', 'merlin-wp') . '
										</span>
									</span>
									</label>
									</li>';
								} elseif ($plugin['type'] == 'iqonic') {
									$iqonic .= '<li data-slug="' . esc_attr($slug) . '">
									<input type="checkbox" name="default_plugins[' . esc_attr($slug) . ']" class="checkbox" id="default_plugins_' . esc_attr($slug) . '" value="1" checked disabled>
									<label for="default_plugins_' . esc_attr($slug) . '">
										<i></i>
										<span>' . esc_html($plugin['name']) . '</span>
						...
						?>
							<?php foreach ($recommended_plugins as $slug => $plugin) :
								if ($plugin['type'] == "woocommerce") {
									$woocommerce .= '<li data-slug="' . esc_attr($slug) . '">
								...
								} elseif ($plugin['type'] == "learnPress") {
									$learnPress .= '<li data-slug="' . esc_attr($slug) . '">
									<input type="checkbox" name="default_plugins[' . esc_attr($slug) . ']" class="checkbox plugin_single_checkbox" id="default_plugins_' . esc_attr($slug) . '" value="1" checked>
									<label for="default_plugins_' . esc_attr($slug) . '">
										<i></i><span>' . esc_html($plugin['name']) . '</span>
									</label>
								</li>';
								} elseif ($plugin['type'] == "gamipress") {
									$gamipress .= '<li data-slug="' . esc_attr($slug) . '">
									<input type="checkbox" name="default_plugins[' . esc_attr($slug) . ']" class="checkbox plugin_single_checkbox" id="default_plugins_' . esc_attr($slug) . '" value="1" checked>
									<label for="default_plugins_' . esc_attr($slug) . '">
										<i></i><span>' . esc_html($plugin['name']) . '</span>
									</label>
								</li>';
								} elseif ($plugin['type'] == "pmp") {
									$pmp .= '<li data-slug="' . esc_attr($slug) . '">
									<input type="checkbox" name="default_plugins[' . esc_attr($slug) . ']" class="checkbox plugin_single_checkbox" id="default_plugins_' . esc_attr($slug) . '" value="1" checked>
									<label for="default_plugins_' . esc_attr($slug) . '">
										<i></i><span>' . esc_html($plugin['name']) . '</span>
									</label>
								</li>';
								} elseif ($plugin['type'] == "bbpress") {
									$bbpress .= '<li data-slug="' . esc_attr($slug) . '">
									<input type="checkbox" name="default_plugins[' . esc_attr($slug) . ']" class="checkbox plugin_single_checkbox" id="default_plugins_' . esc_attr($slug) . '" value="1" checked>
									<label for="default_plugins_' . esc_attr($slug) . '">
										<i></i><span>' . esc_html($plugin['name']) . '</span>
									</label>
									...
									<?php if (!empty($pluginMarkup)) : ?>
										...
										<div class="accordion-item">
											<div class="accordion-item-inner">
												<div class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#<?php echo $section; ?>" aria-expanded="true" aria-controls="collapseOne">
													<h5 class="m-0"><?php echo $section;
																	esc_html_e(' Plugins', 'socialv'); ?></h5>
												</div>
												<div class="check-all-content">
													<div class="form-check m-0">
														<input class="form-check-input plugin-list-dropdown-checkbox" type="checkbox" value="1" id="flexCheckDefault" checked <?php echo (($section == 'Required') || ($section == 'Iqonic')) ? ' disabled' : ''; ?>>

													</div>
												</div>
											</div>
											<ul id="<?php echo $section; ?>" class="list-inline my-2 mx-0 py-0 px-4 accordion-collapse collapse">
	...
	protected function content()
	{
		...
		?>
			<div class="plugin-installation-error">
				...
				<h1>
					<?php esc_html_e("Something went worng !", "socialv"); ?>
				</h1>
				<strong>
					<?php esc_html_e(" Below listed plugin has been not installed. Install it mannually or submit the ticket for Help.", "socialv"); ?>
				</strong>
				<div class="table-required-plugins">
					<div class="table-responsive">
						<table class="table table-striped table-bordered ">
							<thead>
								<tr>
									<th><?php esc_html_e("No", "socialv"); ?></th>
									<th><?php esc_html_e("Required Plugins", "socialv"); ?></th>
									<th><?php esc_html_e("Download Link", "socialv"); ?> </th>
								</tr>
							</thead>
							<tbody>
										...
										<tr>
											...
											<td>
												<span class="download-link">
													<a href="<?php echo esc_url($url) ?>" target="_blank"><?php esc_html_e("Download", "socialv"); ?></a>
												</span>
											</td>
										</tr>
				...
				<div class="plugin-installation-links">
					<a href='<?php echo admin_url('themes.php?page=socialv-setup&step=plugins'); ?>' class='merlin__button merlin__button--knockout merlin__button--no-chevron merlin__button--external'><?php esc_html_e("Try Again", "socialv"); ?></a>
					<a href="https://iqonic.desky.support/" class="merlin__button merlin__button--knockout merlin__button--no-chevron merlin__button--external" target="_blank"><?php esc_html_e('Get Support', 'socialv'); ?></a>
				</div>
			</div>
		<?php
			return;
		...
		//stop action of redirection after plugin activate
		do_action('socialv_stop_redirect_action');
		$strings = $this->strings;
	...
	public function generate_child_functions_php($slug)
	{
		...
		$output = "
			 ...
			 * This child theme was generated by SocialV.
			 *
...
<?php
	...
	public function socialv_download_file()
	{
		if (!isset($_GET['download_item']) || empty($_GET['download_item']) || !is_user_logged_in()) {
			return;
		}

		if (!current_user_can('manage_options') || !isset($_GET['_wpnonce']) || !wp_verify_nonce($_GET['_wpnonce'], 'socialv_download')) {
			wp_die(__('Unauthorized request.', 'socialv'));
		}

		$socialv_file = basename($_GET['download_item']);
		$socialv_filepath = WP_CONTENT_DIR . "/uploads/$socialv_file";

		// Validate file existence
		if (!file_exists($socialv_filepath)) {
			wp_die(__('File not found.', 'socialv'));
		}

		// Validate allowed file types
		$socialv_allowed_types = ['zip']; // Allowed extensions
		$socialv_file_info = wp_check_filetype($socialv_file);
		if (!in_array($socialv_file_info['ext'], $socialv_allowed_types)) {
			wp_die(__('Invalid file type.', 'socialv'));
		}
		
		header('Content-Type: application/octet-stream');
		header('Content-Disposition: attachment; filename="' . $socialv_file . '"');
		readfile($socialv_filepath);
		exit;
	}

	//stop action of redirect 
	public function socialv_stop_redirect_action(){

		//bbpress redirection stop
		remove_action( 'load-plugins.php', 'bbp_do_activation_redirect'  );
